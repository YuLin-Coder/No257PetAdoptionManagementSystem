    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.chongwufenlei != '') searchWhere.chongwufenlei = '%' + this.formSearch.chongwufenlei + '%';
	      params['sfsh'] = '是';
        this.$http.get('chongwulingyang/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/chongwulingyangDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
